...
public class MessageDaoImpl extends BaseDaoImpl<Message, Long> implements MessageDao {
	...
	public Page<Message> findSenderPage(Member sender, Pageable pageable){
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Message> criteriaQuery = criteriaBuilder.createQuery(Message.class);
		Root<Message> root = criteriaQuery.from(Message.class);
		criteriaQuery.select(root);
		...
		restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.isNull(root.get("forMessage")), criteriaBuilder.equal(root.get("isDraft"), false));
		if (sender != null) {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.and(criteriaBuilder.equal(root.get("sender"), sender), criteriaBuilder.equal(root.get("senderDelete"), false)));
		} else {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.and(criteriaBuilder.and(criteriaBuilder.isNull(root.get("sender")), criteriaBuilder.equal(root.get("senderDelete"), false))));
		}
		criteriaQuery.where(restrictions);
		return super.findPage(criteriaQuery, pageable);
	}
	
	public Page<Message> findReceiverPage(Member receiver, Pageable pageable){
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Message> criteriaQuery = criteriaBuilder.createQuery(Message.class);
		Root<Message> root = criteriaQuery.from(Message.class);
		criteriaQuery.select(root);
		Predicate restrictions = criteriaBuilder.conjunction();
		if (receiver != null) {
			restrictions = criteriaBuilder.and(restrictions,criteriaBuilder.equal(root.get("receiver"), receiver), criteriaBuilder.equal(root.get("receiverDelete"), false));
		} else {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.isNull(root.get("receiver")), criteriaBuilder.equal(root.get("receiverDelete"), false));
		}
		criteriaQuery.where(restrictions);
		return super.findPage(criteriaQuery, pageable);
	}

	public void removeX(Long id, Member member , String remark){
		Message message = super.find(id);
		if (message == null || message.getForMessage() != null) {
			return;
		}
		if("inbox".equals(remark) || "outbox".equals(remark)){
		if(member != null && member.equals(message.getReceiver()) && member.equals(message.getSender())){
			if (message.getIsDraft()) {
				super.remove(message);
			} else {
				if (message.getReceiverDelete() && message.getSenderDelete()) {
					super.remove(message);
				} else {
					if("inbox".equals(remark)){
						message.setReceiverDelete(true);
					}else if("outbox".equals(remark)){
						message.setSenderDelete(true);
					}
					super.merge(message);
				}
