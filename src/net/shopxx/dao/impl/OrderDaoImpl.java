...
public class OrderDaoImpl extends BaseDaoImpl<Order, Long> implements OrderDao {
	...
	public Order findBySn(String sn) {
		...
		if (store != null) jpql += " and orders.store=:store";
		try {
			TypedQuery<Order> query = entityManager.createQuery(jpql, Order.class).setFlushMode(FlushModeType.COMMIT).setParameter("sn", sn);
			if (store != null) 
	...
	public List<Order> findList(Member member, Integer count, List<Filter> filters, List<net.shopxx.Order> orders) {
		...
		Store store = WebUtils.getStore();
		if (store != null)
			criteriaQuery.where(criteriaBuilder.equal(root.get("member"), member), criteriaBuilder.equal(root.get("store"), store));
		else
	...
	public Page<Order> findPage(String searchValue,Member member, Pageable pageable) {
		...
		if (null!=searchValue&&!searchValue.equals("")) {
			restrictions = criteriaBuilder.like(root.get("sn").as(String.class), "%"+searchValue+"%");
		}
		Store store = WebUtils.getStore();
		if (store != null) 
	...
	public Page<Order> findUnpaidPage(Member member, Pageable pageable) {
		...
		restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.greaterThanOrEqualTo(root.<Date>get("expire"), new Date()));
		Store store = WebUtils.getStore();
		if (store != null) 
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("store"), store));
		criteriaQuery.where(restrictions); 
	...
	public Page<Order> waitingConmentPage(Member member ,Pageable pageable) {
		...
		restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("orderStatus"), OrderStatus.completed), criteriaBuilder.notEqual(root.get("orderStatus"), OrderStatus.cancelled), criteriaBuilder.equal(root.get("paymentStatus"), PaymentStatus.paid), criteriaBuilder.equal(root.get("shippingStatus"), ShippingStatus.shipped));
		restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.or(root.get("expire").isNull(), criteriaBuilder.greaterThanOrEqualTo(root.<Date> get("expire"), new Date())));
		restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("reviewStatus"),Order.ReviewStatus.unreview));
		if (member != null) {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("member"), member));
		}
		Store store = WebUtils.getStore();
	...
	public void releaseStock() {
		...
		List<Order> orders;
		if (store == null) 
			orders = entityManager.createQuery(jpql, Order.class).setParameter("isAllocatedStock", true).setParameter("now", new Date()).getResultList();
		else
			orders = entityManager.createQuery(jpql, Order.class).setParameter("isAllocatedStock", true).setParameter("now", new Date()).setParameter("store", store).getResultList();
		if (orders != null) {
	...
	public List<Order> findListForDeleteStore(Store store){
		String jpql = "from Order od where od.store = :store";
		return entityManager.createQuery(jpql, Order.class).setParameter("store", store).setFlushMode(FlushModeType.COMMIT).getResultList();
	}
}