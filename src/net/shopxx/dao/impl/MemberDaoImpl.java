...
public class MemberDaoImpl extends BaseDaoImpl<Member, Long> implements MemberDao {
	...
	public Member findByMicrono(String microno) {
		if (microno == null) {
			return null;
		}
		try {
			String jpql = "select members from Member members where lower(members.microno) = lower(:microno)";
			return entityManager.createQuery(jpql, Member.class).setFlushMode(FlushModeType.COMMIT).setParameter("microno", microno).getSingleResult();
		} catch (NoResultException e) {
			return null;
		}
	}

	public List<Member> findListByWechatOpenid(String openid) {
		if (openid == null) {
			return null;
		}
		try {
			String jpql = "select members from Member members where lower(members.openid) = lower(:openid)";
			return entityManager.createQuery(jpql, Member.class).setFlushMode(FlushModeType.COMMIT).setParameter("openid", openid).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	public Member findLoginByUsername(String username) {
		if (username == null) {
			return null;
		}
		try {
			String jpql = "select members from Member members where lower(members.username) = lower(:username) or lower(members.mobile) = lower(:mobile) ";
			return entityManager.createQuery(jpql, Member.class).setFlushMode(FlushModeType.COMMIT).setParameter("username", username).setParameter("mobile", username).getSingleResult();
		} catch (NoResultException e) {
			return null;
		}
	}

	public boolean updatPoint(Long id, BigDecimal point) {
		String jpql="update Member m set m.point="+point+" where lower(m.id)="+id;
