...
public abstract class BaseDaoImpl<T, ID extends Serializable> implements BaseDao<T, ID> {
	...
	public Page<T> findPage(List<Filter> filters, List<Order> orders ,Pageable pageable) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		//entityClass参数指定查询结果类型是entityClass
		CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(entityClass);
//		criteriaQuery.from(entityClass)： 为CriteriaQuery对象指定一个或多个查询源
		//AbstractQuery接口是众多接口中的一员，如CriteriaQuery，From和root
	...
	protected Page<T> findPage(CriteriaQuery<T> criteriaQuery ,List<Filter> filters, List<Order> orders , Pageable pageable) {
		Assert.notNull(criteriaQuery);
		Assert.notNull(criteriaQuery.getSelection());
		Assert.notEmpty(criteriaQuery.getRoots());
