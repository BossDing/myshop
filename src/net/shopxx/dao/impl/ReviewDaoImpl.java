...
public class ReviewDaoImpl extends BaseDaoImpl<Review, Long> implements ReviewDao {
	...
	public boolean isReviewed(Member member, Product product) {
		...
		Long count = 0L;
		String jqpl = "select count(*) from Review review where review.member = :member and review.product = :product and review.forReview is null";
		count = entityManager.createQuery(jqpl, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("member", member).setParameter("product", product).getSingleResult();
		return count > 0;
	}
