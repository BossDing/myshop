...
public class PromotionDaoImpl extends BaseDaoImpl<Promotion, Long> implements PromotionDao {
	...
	public Promotion findByName(String name) {
		...
		String jpql = "select promotion from Promotion promotion where promotion.name = :name";
		return entityManager.createQuery(jpql, Promotion.class).setFlushMode(FlushModeType.COMMIT).setParameter("name", name).setMaxResults(1).getSingleResult();
	}
