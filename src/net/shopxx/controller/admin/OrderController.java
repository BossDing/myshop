...
public class OrderController extends BaseController {
	...
	public String complete(Long id, RedirectAttributes redirectAttributes) {
		...
		if (order != null && !order.isExpired() && order.getOrderStatus() == OrderStatus.confirmed && !order.isLocked(admin)) {
			...
			for(int i=0;i<orderItems.size();i++){
				orderItems.get(i).setReviewStatus(false);
			}
			order.setOrderItems(orderItems);
			order.setReviewStatus(Order.ReviewStatus.unreview);
	 ...
	 * 退积分（仅针对完全积分兑换订单）
	 */
	@RequestMapping(value = "/refundsJf", method = RequestMethod.POST)
	public String refundsJf(Long orderId, Long paymentMethodId, Refunds refunds, RedirectAttributes redirectAttributes) {
		Order order = orderService.find(orderId);
...
//		}
		...
		orderService.refundsJf(order, refunds, admin);
		addFlashMessage(redirectAttributes, SUCCESS_MESSAGE);
		return "redirect:view.jhtml?id=" + orderId;
	}
	...
	public String refunds(Long orderId, Long paymentMethodId, Refunds refunds, RedirectAttributes redirectAttributes) {
		...
		if (store != null && !store.getId().equals(order.getStore().getId())) { //只能操作自家店铺的订单
			return ERROR_VIEW;
		}
		refunds.setOrder(order);
		PaymentMethod paymentMethod = paymentMethodService.find(paymentMethodId);
	...
	Map<String, Object> calculate(Order order, Long areaId, Long paymentMethodId, Long shippingMethodId) {
		...
		order.setStore(pOrder.getStore()); //设置店铺属性
	...
	public String update(Order order, Long areaId, Long paymentMethodId, Long shippingMethodId, RedirectAttributes redirectAttributes) {
		...
		order.setStore(pOrder.getStore()); // cgd 2014-10-22 添加店铺
	...
	public String list(OrderStatus orderStatus, PaymentStatus paymentStatus, ShippingStatus shippingStatus, Boolean hasExpired,String type, Pageable pageable, ModelMap model) {
		model.addAttribute("orderStatus", orderStatus);
		model.addAttribute("paymentStatus", paymentStatus);
		model.addAttribute("shippingStatus", shippingStatus);
		model.addAttribute("hasExpired", hasExpired);
		model.addAttribute("type", type);
		model.addAttribute("page", orderService.findPage(orderStatus, paymentStatus, shippingStatus, hasExpired,type, pageable));
