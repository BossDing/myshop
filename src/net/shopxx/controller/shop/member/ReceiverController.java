...
public class ReceiverController extends BaseController {
	...
	public  @ResponseBody
	Message save(Receiver receiver, Long areaId,String zipCode ,String consignee , String phone ,RedirectAttributes redirectAttributes) {
		receiver.setArea(areaService.find(areaId));
		Member member = memberService.getCurrent();
		if (Receiver.MAX_RECEIVER_COUNT != null && member.getReceivers().size() >= Receiver.MAX_RECEIVER_COUNT) {
			return  Message.error("当前已有20条收货地址");
		}
		if("区号".equals(receiver.getAreaCode())){
			receiver.setAreaCode(null);
		}else if("电话号码".equals(receiver.getTelephone())){
			receiver.setTelephone(null);
		}else if("分机".equals(receiver.getExtension())){
			receiver.setExtension(null);
		}
		receiver.setMember(member);
		if(null==receiver.getIsDefault()){
		...
		return Message.success("shop.receiver.success", receiver);
	}
	/**
	 * 编辑
	 */
	...
	public String update(Receiver receiver, Long id, Long areaId, RedirectAttributes redirectAttributes) {
		System.out.println(id+"_"+areaId+"_"+receiver.getIsDefault());
		receiver.setArea(areaService.find(areaId));
		if (!isValid(receiver)) {
			return ERROR_VIEW;
		}
