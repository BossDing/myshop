...
public class PaymentController extends BaseController {
	...
	public String submit(Type type, String paymentPluginId, String sn, BigDecimal amount, HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		if (type == Type.payment) {
			order = orderService.findBySn(sn);
			...
			if(order.getStore() != null) payment.setStore(order.getStore());
		...
		if(order.getStore() != null) {
			model.addAttribute("parameterMap", paymentPlugin.getParameterMap(payment.getSn(), description, request, order.getStore()));
		} else {
	...
	public String notify(@PathVariable NotifyMethod notifyMethod, @PathVariable String sn, HttpServletRequest request, ModelMap model) {
		...
		if (payment != null) {
			if(order.getStore() != null) {
				paymentPlugin = pluginService.getPaymentPlugin(payment.getPaymentPluginId(), order.getStore());
				if (paymentPlugin != null) {
					if (paymentPlugin.verifyNotify(sn, notifyMethod, request, order.getStore())) {
						paymentService.handle(payment);
					}
					model.addAttribute("notifyMessage", paymentPlugin.getNotifyMessage(sn, notifyMethod, request));
				}
			} else {
				paymentPlugin = pluginService.getPaymentPlugin(payment.getPaymentPluginId());
				if (paymentPlugin != null) {
