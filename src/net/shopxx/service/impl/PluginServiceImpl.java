...
public class PluginServiceImpl implements PluginService {
	...
	public List<PaymentPlugin> getPaymentPlugins(final Store store) {
		...
		CollectionUtils.select(paymentPlugins, new Predicate() {
			public boolean evaluate(Object object) {
				...
				return paymentPlugin.getPluginConfig(store) != null;
			}
		}, result);
	...
	public PaymentPlugin getPaymentPlugin(String id, Store store) {
		for(int i = 0; i< paymentPluginMap.size(); i++) {
			PaymentPlugin paymentPlugin = paymentPluginMap.get(id);
			if(paymentPlugin.getPluginConfig(store).getId() > 0) {
