...
public class StaticServiceImpl implements StaticService, ServletContextAware {
	...
	Set<ParameterGroup> parameterGroups(Long id) {
		...
		if(productCategory.getParameterGroups()==null || productCategory.getParameterGroups().size()<=0){
			...
			for (Iterator iterator = list.iterator(); iterator.hasNext();) {
				...
				if(p.getParameterGroups()!=null && p.getParameterGroups().size()>0){
					for (Iterator iterator2 = p.getParameterGroups().iterator(); iterator2.hasNext();) {
						ParameterGroup name = (ParameterGroup) iterator2.next();
						if(name.getParameters()!=null && name.getParameters().size()>0){
	...
	public int build(Product product) {
		...
		Template pointtemplate = templateService.get("pointProductContent");
		Template mobiletemplate = templateService.get("mobileProductContent");
		Template gwtemplate = templateService.get("gwProductContent");
		int buildCount = 0;
		if (product.getIsMarketable()) {
			Map<String, Object> model = new HashMap<String, Object>();
			model.put("product", product);
			
			/**创建积分商品详情-zlh-405为积分标签id*/
			Set<Tag> tags = product.getTags();
			Boolean arg = false;
			if(null!=tags&&tags.size()>0){
				for(Tag t :tags){
					if(t.getId()==405||t.getName().equals("积分")){
						arg = true;
					}
				}
			}
			if(arg){
				build(pointtemplate.getTemplatePath(), product.getPointPath(), model);
			}
			//静态手机端商城 edit by xiaolai 2014-04-18  
			if("macrogw".equals(product.getEntcode())){
				Set<ParameterGroup> s = parameterGroups(product.getProductCategory().getId());
				if(product.getId().intValue()==314){
					for (Iterator iterator = s.iterator(); iterator.hasNext();) {
						ParameterGroup name = (ParameterGroup) iterator.next();
					}
				}
				model.put("parameterGroups", s);
				build(gwtemplate.getTemplatePath(), product.getGwPath(), model); 
			}else{
				build(mobiletemplate.getTemplatePath(), product.getMobilePath(), model); 
	...
	@Transactional(readOnly = true)
	public int build_Mobile(Product product) {
		Assert.notNull(product);
	...
	@Transactional(readOnly = true)
	public int buildMobile() {
		int buildCount = 0;
		for (int i = 0; i < productDao.count(); i += 20) {
			List<Product> products = productDao.findList(i, 20, null, null);
			for (Product product : products) {
				buildCount += build_Mobile(product);
			}
			productDao.clear();
		}
		return buildCount;
