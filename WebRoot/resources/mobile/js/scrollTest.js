	...
	var Zepto = (function() {
		...
		["width", "height"].forEach(function(dimension) {
			$.fn[dimension] = function(value) {
				...
				if (value === undefined) {
					return this[0] == window ? window["inner" + Dimension] : this[0] == document ? document.documentElement["offset" + Dimension] : (offset = this.offset()) && offset[dimension]
				} else {
					return this.each(function(idx) {
						var el = $(this);
	...
	if (!a.seajs) {
		...
		s.prototype.resolve = function() {
			...
			if (!(a.status >= $.LOADING)) {
				...
				var g = {};
				for (f = 0; d > f; f++) {
					e = V[c[f]], e.status < $.FETCHING ? e.fetch(g) : e.status === $.SAVED && e.load()
				}
				for (var h in g) {
			...
			var b = a._waitings,
				c, d;
			for (c in b) {
				b.hasOwnProperty(c) && (d = V[c], d._remain -= b[c], 0 === d._remain && d.onload())
			}
			...
			for (var b in a) {
				...
				if (d && v(d)) {
					...
					x(d) ? c = d.concat(c) : "base" === b && ("/" !== c.slice(-1) && (c += "/"), c = l(c)), u[b] = c
				}
			}
			return B("config", a), t
		}
