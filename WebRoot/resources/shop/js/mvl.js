        ...
        $(document).ready(
            function () {
                ...
                $("#xyddul li").click(
                    function () {
                        if (!$("#zhezhao").is(":animated")) {
                            $("#ImageList ul li").eq(nowshowpic).animate({ "opacity": 0 }, 3000);
                            nowshowpic = $(this).index();
                            $("#ImageList ul li").eq(nowshowpic).animate({ "opacity": 1 }, 3000);
                            shezhixiaoyuandian(nowshowpic);
                        }
                    }
                );
                ...
                function huxixianshitupian(a) {
                    $("#ImageList ul li").eq(a - 1).animate({ "opacity": 0 }, 3000);
                    $("#ImageList ul li").eq(a).animate({ "opacity": 1 }, 3000);

                }
                function huxixianshitupian2(a) {
                    if (a == $("#ImageList ul li").length - 1) {
                        $("#ImageList ul li").eq(0).animate({ "opacity": 0 }, 3000);
                        $("#ImageList ul li").eq(a).animate({ "opacity": 1 }, 3000);
                    } else {
                        $("#ImageList ul li").eq(a + 1).animate({ "opacity": 0 }, 3000);
                        $("#ImageList ul li").eq(a).animate({ "opacity": 1 }, 3000);
                ...
                var timer = window.setInterval(method, 4000);                               /*设置自动轮播*/
                var nowshowpic = 0;
                ...
                $('#banner').mouseleave(function () {
                    timer = window.setInterval(method, 4000);
                });
