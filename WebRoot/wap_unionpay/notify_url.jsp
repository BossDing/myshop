...
<%@ page import="net.shopxx.plugin.wap_unionpay.util.*"%>
<%
	//获取银联POST过来异步通知信息
	...
	if(UpmpService.verifySignature(params)){// 服务器签名验证成功
		...
		if (null != transStatus && transStatus.equals("00")) {
		    // 交易处理成功
// 		MypayUtil.verify(orderSn);
		response.sendRedirect("http://192.168.101.1:9008/mobile/index.jhtml"); 
		} else {
		out.println("false1");
		}
		out.println("success");
	}else{// 服务器签名验证失败
		response.sendRedirect("http://192.168.101.1:9008/ndex.jhtml"); 
	}
%>
