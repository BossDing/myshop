					...
					$.ajax({
						...
						success: function(message) {
							...
							if (message.type == "success") {
								alert("亲!您的微信号已与您的账号解绑成功, 您可使用绑定其他账号!");
								location.href = "${base}/mobilelogout.jhtml";
							} else {
								$.message(message);
							}
						}
...
<div class="Big">
	<form id="loginForm" action="${base}/mobile/bind/unbind.jhtml" method="post">
	    ...
	    <div class="KK">
	    	...
	    	<input type="submit" class="Big-denglu" value="解除绑定"/>
	    	<div class="aa">亲,您的微信号已经绑定了一个奥马账号,</div><div class="bb">您如果想绑定另一个账号,需先将之前的账号解绑,&nbsp;&nbsp;感谢您的合作!</div>
	    </div>
    </form>
</div>
