...
$().ready(function() {
	...
	$addToCart.click(function() {
	  if( $('#is_inv').val()<1){
		...
		[/#if]
		  var quantity = $quantity.val();
		  if (/^\d*[1-9]\d*$/.test(quantity) && parseInt(quantity) > 0) {
		    var args = ${product.id}+"a"+quantity;
			location.href="${base}/member/order/inJf/"+args+".jhtml";
		  } else {
			$.message("warn", "${message("shop.product.quantityPositive")}");
		}
	});
	...
	$window.scroll(function() {
		...
		if (scrollTop > barTop) {
			...
			$bar.find("li").removeClass("current");
			$bar.css({position: "absolute", top: "760px"});
		}
	});
...
<body style="background:#f8f8f8;">
	...
	<div class="span18 last" style="width:1050px; margin-top:30px; height:530px; margin-bottom:20px;">
			...
			<div style=" width: 598px; float: right; padding-top: 10px; font-size: 14px; ">
				<dl>
			...
			<div class="info">
				...
				<dl>
					<dt>兑换价:</dt>
					<dd>
						<strong>￥${product.lowPrice}</strong>
					</dd>
				</dl>
				[#if product.lowPoints > 0]
					<dl>
						<dt>兑换积分:</dt>
						<dd>
							<span>${product.lowPoints}</span>
						</dd>
					</dl>
				[/#if]
			</div>
			[#if !product.isGift]
				<div class="action">
					...
					<div class="buy">
						...
						[#else]
							<input type="button" id="addToCart" class="addCart" value="立即购买" />
						[/#if]
		...
		<div class="span18 last" style="width:800px;">
					...
					<div class="content">
						[@consultation_list productId = product.id count = 5]
							[#if consultations?has_content]
								...
								<p>
									<a href="${base}/consultation/add/${product.id}.jhtml" id="addConsultation">立即购买</a>
