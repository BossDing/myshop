...
$().ready(function() {
	...
	$submit.click(function() {
		...
		if($productId==null||$productId.val()==null||$productId.val()==""){
		    $.message("warn", "${message("未选择商品")}");
			return false;
		}
		var $quantity = $("#quantity");
		if(${order.member.point}<(${product.lowPoints}*$quantity.val())){
		    $.message("warn", "${message("积分不足，请选择其他商品")}");
			return false;
		}
			...
			<div class="info">
				...
				<form id="orderForm" action="create.jhtml" method="post">
					...
					<table class="product">
						...
						[#list order.orderItems as orderItem]
							<tr>
								...
								<td>
									[#if !orderItem.isGift]
										${currency(product.lowPrice, true)}
									[#else]
										-
									[/#if]
								</td>
								...
								<td>
									${orderItem.quantity}
										<input type="hidden" name="quantity" value="${orderItem.quantity}" id="quantity"/>
								</td>
								<td>
									[#if !orderItem.isGift]
										价格小计：${currency(orderItem.lowPrice*orderItem.quantity, true)} &nbsp;
										积分小计: ${orderItem.lowPoints*orderItem.quantity}
									[#else]
										-
									[/#if]
								</td>
					...
					<div class="span12">
						...
						<input type="hidden" id="code" name="code" maxlength="200" value="" />
					</div>
					...
					<div class="span24">
						<div class="bottom">
							<a href="${base}/product/list.jhtml?tagIds=405" class="back">返回积分商城</a>
