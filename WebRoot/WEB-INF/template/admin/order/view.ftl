	...
	[#if !order.expired && order.orderStatus == "confirmed" && (order.paymentStatus == "paid" || order.paymentStatus == "partialPayment" || order.paymentStatus == "partialRefunds")]
		...
		$refundsButton.click(function() {
			$.dialog({
				...
				content: '
					<form id="refundsJfForm" action="refundsJf.jhtml" method="post">
					<input type="hidden" name="orderId" value="${order.id}" \/>
								<input type="hidden" name="token" value="${token}" \/>
					<table class="input">
					<tr>
				...
				[#else]
					...
					<form id="refundsForm" action="refunds.jhtml" method="post">
						...
						<table class="input">
							...
							<\/tr>
                              [/#if]
                             [#elseif order.type?? && order.type=="0"]	
	                         <tr>
								<th>
									${message("Order.amount")}:
								<\/th>
...
<body>
	...
	<table class="input tabContent">
		...
		<tr>
			...
			[#if order.type?? &&order.type=="1"]
		      [#if order.amount <= 0]
						   ${order.lowPoints} 积分
		      [#else]
		       ${currency(order.amount, true)} &nbsp;  ${order.lowPoints} 积分
		      [/#if]
			   [#else]
				${currency(order.amount, true)}
			 [/#if]
			...
			[#if order.type?? &&order.type=="1"]
		      [#if order.amount <= 0]
						   ${order.lowPoints} 积分
		      [#else]
		         ${currency(order.amountPaid, true)} &nbsp;  ${order.lowPointsPaid} 积分
		      [/#if]
			   [#else]
				${currency(order.amountPaid, true)}
			 [/#if]
        ...
        <tr>
         	<th>
				${message("admin.order.storeBelong")}:
			</th>
			<td>
                [#if order.store??]${order.store.name}[#else] - [/#if]
			</td>    
        </tr>
	</table>
	<table class="input tabContent">
		...
		[#list order.orderItems as orderItem]
			<tr>
				...
				<td>
					...
					[#if order.type?? &&order.type=="1"]
					    [#if order.amount <= 0]
						   ${orderItem.lowPoints} 积分
						[#else]
						   ${currency(orderItem.lowPrice, true)} &nbsp;  ${orderItem.lowPoints} 积分
						[/#if]
					[#else]	
						   ${currency(orderItem.lowPrice, true)}
					[/#if]
					<!--结束-->
					[#else]
						-
				...
				<td>
					...
					[#if order.type?? &&order.type=="1"]
					    [#if order.amount <= 0]
						   ${orderItem.subtotalLowPoints} 积分
						[#else]
						   ${currency(orderItem.subtotalLowPrice, true)} &nbsp;  ${orderItem.subtotalLowPoints} 积分
						[/#if]
					[#else]	
						   ${currency(orderItem.subtotalLowPrice, true)}
					[/#if]
					<!--结束-->
					[#else]
						-
		...
		[#list order.payments as payment]
			<tr>
				...
				<td>
				 	[#if order.type?? &&order.type=="1"]
					    [#if order.amount <= 0]
						   ${order.lowPointsPaid} 积分
						[#else]
						 	${currency(payment.amount, true)}&nbsp;  ${order.lowPointsPaid} 积分
						[/#if]
					[#else]	
