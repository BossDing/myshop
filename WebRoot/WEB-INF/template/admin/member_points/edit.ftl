...
$().ready(function() {
	...
	var serialSignInTimesValid=true;//连续签到次数是否验证通过。
	var pointValid=true;//积分是否验证通过
	...
	function checkSerialSignInTimes(){
		...
		if(reg.test($inputSerialSignInTimes.val())){
			$.get("check_serialSignInTimes.jhtml?previousSerialSignInTimes="+${signInPointRule.serialSignInTimes}+"&serialSignInTimes="+$inputSerialSignInTimes.val(),function(data){
					if(data==true){//校验通过
	...
	function checkPoint(){
		...
		if(reg.test($inputPoint.val())){
			if($inputSerialSignInTimes.hasClass('valid')||$inputSerialSignInTimes.val()==${signInPointRule.serialSignInTimes}){
				$.get("check_point.jhtml?previousSerialSignInTimes="+${signInPointRule.serialSignInTimes}+"&serialSignInTimes="+$inputSerialSignInTimes.val()+"&point="+$inputPoint.val(),
					function(data){
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; 编辑签到积分奖励
	</div>
	<form id="inputForm" action="update.jhtml" method="post">
		<input type="hidden" name="id" value="${signInPointRule.id}" />
		<table class="input">
			<tr>
				<th>
				...
				<td>
					<input type="text" name="serialSignInTimes" value="${signInPointRule.serialSignInTimes}" class="text" maxlength="100" />
			...
			<tr>
				...
				<td>
					<input type="text" name="point" value="${signInPointRule.point}" class="text" maxlength="100" />
