...
$().ready(function() {
	...
	// 覆盖区域
	$is_area_name.click(function() {
		if ($(this).prop("checked")) {
			$area_show.show();
			$all_checked_button.show();
			$is_area_name.prop("disabled", false);
		} else {
			$area_show.hide();
			$all_checked_button.hide();
		}
	});
	
	//全选
	$all_checked.click(function() {
		var el=document.getElementsByName("area_name"); 
	...
	//全不选
	$all_not_checked.click(function() {
		var el=document.getElementsByName("area_name"); 
		for (var i=0;i<el.length;i++) { 
			if(el[i].type=="checkbox") { 
				el[i].checked=false; 
	...
	<form id="inputForm" action="update.jhtml" method="post" enctype="multipart/form-data">
		...
		<table class="input tabContent">
				...
				<td>
					<select id="productCategoryId" name="productCategoryId">
						[#list productCategoryTree as productCategory]
	                        [#if store == null && product.store == null && productCategory.store == null][#--非店铺商品--]
	                        ...
	                        [#elseif store == null && product.store != null][#--店铺商品--]
		                        [#if productCategory.store == null][#--非店铺分类--]
		                        	<option value="${productCategory.id}"[#if productCategory == product.productCategory] selected="selected"[/#if]>
										[#if productCategory.grade != 0]
											[#list 1..productCategory.grade as i]
												&nbsp;&nbsp;
	                        	...
	                        	[#elseif productCategory.store.id == product.store.id][#--对应店铺的分类--]
		                        	<option value="${productCategory.id}"[#if productCategory == product.productCategory] selected="selected"[/#if]>
										[#if productCategory.grade != 0]
											[#list 1..productCategory.grade as i]
												&nbsp;&nbsp;
			...
			<tr>
				...
				<td>
					<input type="text" name="order" class="text" value="${product.order}" maxlength="9" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="text" name="orderNew" class="text" value="${product.orderNew}" maxlength="9" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="text" name="orderHot" class="text" value="${product.orderHot}" maxlength="9" />
				</td>
			</tr>
			[/#if]
			<tr>
			...
			<tr>
				...
				<td>
					<label>
						<input type="checkbox" id="is_area_name" name="is_area_name" value="true" />选择覆盖区域
					</label>
					<label  id="all_checked_button">
						<input type="radio" id="all_checked" name="all_checked" value="true" />全选
						<input type="radio" id="all_not_checked" name="all_checked" value="true" />全否
					</label>
				</td>
			</tr>
			<tr id="area_show" class="hidden">
			<th></th>
			<td>
					[#assign x=0 /]
					[#list areas as area]
					[#assign x=x+1 /]
					[#if x%7==0]</br>[/#if]
						<input type="checkbox" name="area_name" value="${area.id}" [#if product.areas?seq_contains(area)] checked="checked"[/#if]/>${area.name}
					[/#list]
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<span class="fieldSet">
						<input type="text" name="bigimage" class="text" value="${product.bigimage}" maxlength="200" title="${message("admin.product.imageTitle")}" />
						<input type="button" id="browserButton2" class="button" value="${message("admin.browser.select")}" />
						[#if product.bigimage??]
							<a href="${product.bigimage}" target="_blank">${message("admin.common.view")}</a>
						[/#if]
					</span>  
				</td>
			</tr>
			<tr>
				...
				<td>
					<span class="fieldSet">
						<input type="text" name="instruction" class="text" value="${product.instruction}" maxlength="200" title="${message("admin.product.imageTitle")}" />
						<input type="button" id="browserButton1" class="button" value="${message("admin.browser.select")}" />
						[#if product.instruction??]
							<a href="${product.instruction}" target="_blank">${message("admin.common.view")}</a>
						[/#if]	
					</span>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					[#if store == null]
						<label>
							<input type="checkbox" name="isUseFenxiao" value="true" [#if product.isUseFenxiao] checked="checked"[/#if]/>是否分销
							<input type="hidden" name="_isUseFenxiao" value="false" />
						</label>
					[/#if]
                    [#if product.checkStatus != "waitting" && product.checkStatus != "failure"][#-- 可以设置是否上架，若果该商品审核状态不为：待审核、未通过 --]
					<label>
                    ...
                    [#else]
                    <input type="hidden" name="isMarketable" value="false"/>[#-- 店铺商品默认未上架（等待审核） --]
                    [/#if]
                    <input type="hidden" name="checkStatus" value="${product.checkStatus}"/>
					<label>
			...
			<tr>
				...
				<td>
					<input type="text" name="lowPrice" class="text" value="${product.lowPrice}" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="text" name="lowPoints" class="text" value="${product.lowPoints}" />
				</td>
			</tr>
			<tr>
				<th>
            ...
            <tr>
				...
				<td>
	                [#if product.store??]${product.store.name}[#else] - [/#if]
				</td>    
        	</tr>
		</table>
		<table class="input tabContent">
			...
			<tr>
				<td>
					<table id="specificationProductTable" class="input">
						...
						[/#if]
                        [#if product.siblings?has_content]
		...
		<table class="input">
			<tr>
				...
				<td>
                    [#if store == null && product.store == null][#--非店铺管理员不能编辑店铺的商品--]
